                ...
                [id]: {
                    ...
                    position: state.windows[id]?.position || (() => {
                        if (typeof window !== 'undefined' && id === 'messages') {
                            const width = 500; // Reduced for 60% zoom
                            const height = 400; // Reduced for 60% zoom
                            return {
                                x: Math.max(0, (window.innerWidth - width) / 2),
                                y: Math.max(0, (window.innerHeight - height) / 2 + 5)
                            };
                        }
                        return { x: 50 + (newZIndex % 10) * 20, y: 30 + (newZIndex % 10) * 20 };
                    })(),
                    size: state.windows[id]?.size || (id === 'projects' ? { width: 700, height: 500 } : { width: 550, height: 400 }),
